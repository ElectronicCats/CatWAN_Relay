...
unsigned long oledClearTime = 0; // Tiempo para limpiar la pantalla OLED
...
void loop() {
  ...
  if (oledClearTime > 0 && millis() > oledClearTime) {
    oledManager.clear();
    oledClearTime = 0;
  }
...
void setupWiFi() {
  ...
  WiFiManagerParameter custom_n8n_url("n8nurl", "URL Webhook n8n (Logs)",
                                      n8n_url, 150);

  WiFiManagerParameter custom_n8n_time_url(
      "n8ntimeurl", "URL Webhook n8n (Hora)", n8n_time_url, 150);
...
void toggleSolderingIron(int relayIndex, String cardUID) {
  ...
  unsigned long duration =
      newState ? 0 : relayManager.getRelayUptime(relayIndex);
  ...
  if (newState) {
    oledManager.showWelcomeName(cardName);
  } else {
    oledManager.showGoodbyeName(cardName);
  }

  oledClearTime = millis() + 3000; // Limpiar pantalla en 3 segundos
  ...
  String jsonPayload = "{\"uid\":\"" + cardUID + "\", \"act\":\"" + action +
...
void handleSerialCommands() {
  ...
  command.trim();
  command.replace("\r", ""); // Remove any carriage return character
  command.toUpperCase();
  ...
  if (waitingForCardName) {
    ...
    return; // ← Return here, so we don't process it as a normal command
  }
  ...
  if (command == CMD_HELP) {
    ...
    Serial.println(
        "¿Eliminar todas las tarjetas? Escribe 'YES' para confirmar");
    delay(2000);
    if (Serial.available()) {
    ...
    Serial.println(
        "Comando no reconocido. Escribe 'HELP' para ver comandos disponibles");
  }
}
